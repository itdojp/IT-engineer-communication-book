...
[OK] **システム的キャリア設計**：アーキテクチャ思考によるキャリア・ポートフォリオ構築  
[OK] **データ駆動成長管理**：メトリクスとアナリティクスによる客観的キャリア最適化  
[OK] **技術ブランド構築**：エンジニアの強みを活かした差別化されたセルフブランディング  
[OK] **持続可能な影響力**：長期的レガシー構築と知識継承システム
...
## 実践：自分の価値提案を「1分/300字」で作り、フィードバックをもらう

### 目的

- 自分の強みと提供価値を言語化し、伝えるコストを下げます
- フィードバックを取り込み、再現性のあるキャリア改善ループを作ります
...
- 直近半年〜1年の成果（3件）
- 想定読者（上長、面談相手、社外コミュニティなど）

### 手順（5ステップ以内）

1. 成果を3件列挙し、「背景/行動/結果」をそれぞれ1〜2行で書いてください（事実ベース）
2. 3件に共通する強み（例：構造化、調整、改善、運用）を2つ抽出してください
3. 「1分版（口頭）」と「300字版（文章）」の自己紹介を作ってください（結論→根拠→具体例の順が崩れないようにする）
4. 相手に確認したい観点を最大3つに絞り、フィードバック依頼文を作ってください（必要なら [付録2：テンプレート・フォーマット集](../appendix-02-templates/) の「T9. フィードバック（SBI）」を参照）
5. 面談や1on1で共有し、次の改善点を1つだけ決めてください（必要なら「T10. 1on1アジェンダ」を使用）
...
- 1分で言い切れるか（時間内に収まるか）
- フィードバックで指摘された「曖昧さ」の件数
- 次の改善点を実行できたか（2週間後に自己評価）

### 典型失敗とリカバリ

- 失敗：抽象的で伝わらない → リカバリ：具体例（何をどう変えたか）を1つ入れる
- 失敗：盛りすぎる → リカバリ：一次情報（チケット/PR/数値）に戻して事実を優先する

### 次の一手

- 合意形成や利害調整が必要になった → シリーズ：[エンジニアの交渉力アーキテクチャ](https://itdojp.github.io/negotiation-for-engineers-book/)
