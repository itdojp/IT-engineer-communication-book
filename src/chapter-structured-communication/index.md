...
<span id="figure-09" aria-hidden="true"></span>
...
<span id="figure-10" aria-hidden="true"></span>
...
## 実践：非同期依頼を「Context Package」で書き、往復を減らす

### 目的

- 相手の不足コンテキストを補い、確認質問を減らします
- 依頼の成果物と期限を明確にします

### 前提

- 依頼したいタスク（レビュー/判断/作業）が1つある

### 入力

- 依頼したい内容（目的/成果物/期限/制約）
- 関連リンク（チケット、仕様、PRなど）

### 手順（5ステップ以内）

1. [付録2：テンプレート・フォーマット集](../appendix-02-templates/) の「T4. 非同期依頼」（依頼が単発/軽量な場合）または「T1. Context Package」（背景や意思決定が必要な場合）を選び、雛形を貼り付けてください
2. 目的/成果物/期限/緊急度/前提リンクを埋めてください
3. 相手に求める観点（最大3つ）を明示してください
4. [付録1：実践チェックリスト集](../appendix-01-checklists/) の「A3. 非同期依頼チェックリスト」で抜け漏れを確認してください
5. 送信後、合意事項を成果物（チケット/議事録）に反映してください（放置しない）

### 成果物

- 非同期依頼文（コピペ可能な形）
- 参照リンク一覧（Context Package）
...
- 失敗：成果物が曖昧 → リカバリ：「文章/判断/作業」のどれかに固定する
- 失敗：期限だけ書いて背景がない → リカバリ：期限の根拠と優先度を追記する

### 次の一手

- PRレビュー依頼を型にしたい → [付録2：テンプレート・フォーマット集](../appendix-02-templates/) の「T5. PRレビュー依頼」＋ [付録1：実践チェックリスト集](../appendix-01-checklists/) の「A4」
- 会議で決める必要がある → [付録2：テンプレート・フォーマット集](../appendix-02-templates/) の「T2/T3」
...
迷ったら次章（第4章）から読み進めてください: [ステークホルダー別コミュニケーション戦略](../chapter-stakeholder-communication/)
