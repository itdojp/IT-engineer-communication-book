...
| system observation | 報告経路、報告が受領確認されるまでの時間、会議構造などのprocess frictionを確認する | 心理的安全性、感情、意欲をscore・推定しない |
...
本書の最小surveyは、Edmondson (1999)が示した7つの観点を日本語で要約した次のdiscussion promptを使います。これは原尺度の公認翻訳や学術的な尺度再現ではなく、チーム改善の対話に用いる最小例です。

| ID | 本人の認識を尋ねるprompt | 方向 |
|---|---|---|
| PS-1 | このチームでは、失敗を理由に責められる懸念があるか | 逆方向 |
| PS-2 | 問題や難しい論点を持ち出せるか | 順方向 |
| PS-3 | 他者との違いを理由に拒まれる懸念があるか | 逆方向 |
| PS-4 | 対人リスクを伴う質問・提案・異論を出せるか | 順方向 |
| PS-5 | 他のメンバーへ助けを求めにくいか | 逆方向 |
| PS-6 | 自分の仕事を意図的に妨げられる懸念があるか | 逆方向 |
| PS-7 | 自分の技能や強みが尊重され、活用されているか | 順方向 |

回答尺度は全項目で共通の5段階（1: 全くそう思わない、3: どちらともいえない、5: 非常にそう思う。2と4は中間）とし、「回答しない」を別に用意します。逆方向項目の変換規則は収集前に固定しますが、本書の最小運用では項目を合算した総合score、合否、固定thresholdを作りません。

集約結果は、事前に定めた最低回答数を満たす場合だけ、項目ごとの回答分布としてteamへ返します。欠損を補完せず、平均値だけへ圧縮せず、異なる項目を相殺しません。単一回の集約結果を「チームの心理的安全性level」やshared beliefの確定値とは呼ばず、「回答者の認識の分布」として対話します。研究・組織調査としてteam-level尺度を推定する場合は、利用権限を確認した原尺度または妥当性を検証した翻訳、公開されたscoring方法、team内合意の分析方法を測定前に専門家と定めます。
...
| 非回答 | 送達障害などsurvey運用の確認だけに使い、心理的安全性や改善要否を推定しない。個人へ回答を催促しない |
...
| action | 単一scoreや固定thresholdではなく、回答分布、対話、業務文脈からチームが改善案を選ぶ |

回答前に、最低回答数、集計owner、共有先、raw回答と集約結果の削除日を通知します。最低回答数を満たさない場合は値を表示せず、期間や対象teamを安易に結合しません。回答率や非回答理由から心理状態を推定しません。集約値が良好でも本人の懸念を否定せず、低い値でも個人の責任にしません。
