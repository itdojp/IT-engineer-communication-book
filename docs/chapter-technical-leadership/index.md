...
| スキル項目 | 現在 | 目標 | 期限 | 学習方法 |
|------------|------|------|------|----------|
| DB 設計 | L1 | L2 | 2ヶ月 | 既存システム分析 |
| API 設計 | L0 | L1 | 1ヶ月 | OpenAPI 仕様作成 |
...
### 学習目標管理
**SMART Goals適用**：
- Specific: Spring Boot Web API の理解
- Measurable: 実装課題5個完了
...
[OK] **技術的意思決定**：ADR ベースの構造化された決定プロセス<br>
[OK] **合意形成**：エンジニアリング的アプローチによる効率的な合意  
[OK] **メンタリング**：システマティックな人材育成フレームワーク  
[OK] **プロジェクト管理**：分散システム思考による最適化手法
...
## 実践：意思決定を「会議→議事録→ADR」で残す

### 目的

- 意思決定の再議論を減らし、説明責任を担保します
- 合意と次アクションを成果物として残します

### 前提

- 決めたいテーマ（設計方針、運用変更、優先度調整など）が1つある
...
1. [付録2：テンプレート・フォーマット集](../appendix-02-templates/) の「T2. 会議アジェンダ」を使い、決めたい論点を最大3つに絞ってください
2. 選択肢と意思決定基準を整理してください（必要なら「T6. 仕様変更提案（RFC）」または「T7. ADR」）
3. 会議を実施し、決定事項と宿題を [付録2：テンプレート・フォーマット集](../appendix-02-templates/) の「T3. 議事録」で残してください
4. 決定事項を「T7. ADR」に確定し、リンクを共有してください
5. 宿題を担当/期限つきでチケット化し、追跡できる状態にしてください
...
- 同じ論点の再議論回数（例：同一テーマの会議回数）
- 決定までのリードタイム（例：初回提案→決定）
- 宿題の期限遵守率

### 典型失敗とリカバリ

- 失敗：決定者が不明で結論が出ない → リカバリ：決定者を明示し、「決めないこと」も宣言する
- 失敗：議事録が残らず、後から揉める → リカバリ：その場で議事録を作り、合意確認してから解散する

### 次の一手

- 利害調整が必要な状況へ進む → [付録2：テンプレート・フォーマット集](../appendix-02-templates/) の「T11. 交渉前の論点整理シート」＋ シリーズ：[エンジニアの交渉力アーキテクチャ](https://itdojp.github.io/negotiation-for-engineers-book/)
- 個人キャリアに接続する → 第10章「[キャリア開発とセルフブランディング](../chapter-career-development/)」
...
- **組織全体に影響したい（利害調整が絡む）** → シリーズ：[エンジニアの交渉力アーキテクチャ](https://itdojp.github.io/negotiation-for-engineers-book/)
