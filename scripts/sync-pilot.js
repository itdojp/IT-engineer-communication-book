...
if(args.length && !args.every((arg)=>arg==='--check'||arg==='--write') || args.includes('--check')&&args.includes('--write')){console.error('Usage: node scripts/sync-pilot.js [--check|--write]');process.exit(2);}
const write=args.includes('--write'); const differences=[];
